package net.onlineshop.core.freemarker.front;
...
//		}
	...
	public void helps() throws Exception {
		...
		if(notices==null || notices.size()==0){
			logger.debug("notices size = 0");
			return;
		}
		logger.debug("notices size = " + notices.size());
		...
		for(int i=0;i<notices.size();i++){
			...
			logger.debug("生成html页面成功！id="+news.getId());
		}
	}
	...
	public String products() throws Exception {
		...
		if(productList==null || productList.size()==0){
			logger.debug("productList size = 0");
			return null;
		}
		...
		for(int i=0;i<productList.size();i++){
			...
			try {
				...
				e.printStackTrace();
				logger.debug("生成html页面失败！id="+p.getId());
				
				errorBuff.append(p.getId()+",");
				continue;
			}
			logger.debug("生成html页面成功！id="+p.getId());
		}
	...
	public String staticProductByID(String id) throws Exception {
		...
		if(p==null || StringUtils.isBlank(p.getProductHTML())){
			logger.debug("ERROR,not found product by id = " + id);
	...
	public String staticNewsByID(String id) throws Exception {
		...
		if(news==null || StringUtils.isBlank(news.getContent())){
			logger.debug("ERROR,not found news by id = " + id);
