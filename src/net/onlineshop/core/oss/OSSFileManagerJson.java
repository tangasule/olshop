package net.onlineshop.core.oss;
...
//		}
		...
		logger.debug("_tmpRootPath="+_tmpRootPath);
		...
		if((osslist.getCommonPrefixes()==null || osslist.getCommonPrefixes().size()==0) && 
			...
			logger.debug("没有文件夹和目录！");
		}else{
			//遍历所有文件夹CommonPrefix
            logger.debug("文件夹CommonPrefixs:");
...
//	                System.out.println(objectSummary.getKey()+","+objectSummary.getETag());
            	Hashtable<String, Object> hash = new Hashtable<String, Object>();
            	logger.debug("objectSummary.getKey()="+objectSummary.getKey());
		...
		logger.debug("result.toJSONString()="+result.toJSONString());
...
//	}
    ...
    private static void ensureBucket(OSSClient client, String bucketName)
        ...
        if (client.isBucketExist(bucketName)){
        	logger.debug("isBucketExist true");
            return;
        }
