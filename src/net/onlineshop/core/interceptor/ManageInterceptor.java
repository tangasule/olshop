...
import net.onlineshop.web.action.manage.system.MenuAction;
import net.onlineshop.web.action.manage.system.UserAction;
...
public class ManageInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation actionInvocation) throws Exception {
		logger.debug("ManageInterceptor.intercept...");
		try {
	...
	private String intercept0(ActionInvocation actionInvocation) throws Exception {
		...
		logger.debug(">>ManageInterceptor==actionName="+actionName+";method="+method+";action="+action+";namespace="+namespace);
//		logger.debug(">>ManageInterceptor==actionName="+actionName+";method="+method+";action="+action+";namespace="+namespace+";action2="+action2);
...
//			logger.debug("不对用户登陆进行拦截。不对加载权限资源进行拦截");
...
//			logger.debug("optionMethod="+dbMethod);
			if(dbMethod!=null){
				logger.debug("action-method权限检查：user.getDbPrivilegeMap()="+user.getDbPrivilegeMap()+",method="+method);
				logger.debug("user.getDbPrivilegeMap().get(optionMethod)="+user.getDbPrivilegeMap().get(dbMethod));
				...
				if(StringUtils.isBlank(op)){
					...
					if(dbMethod.equals(ManageContainer.db_insert)){
						logger.debug(ManageContainer.db_privilege_insert_error);
						...
						logger.debug(ManageContainer.db_privilege_update_error);
						...
						logger.debug(ManageContainer.db_privilege_delete_error);
		...
		logger.debug("ManageInterceptor.actionInvocation.invoke().");
		return actionInvocation.invoke();
