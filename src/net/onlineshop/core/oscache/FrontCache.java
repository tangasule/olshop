package net.onlineshop.core.oscache;
...
import net.onlineshop.services.front.commentType.CommentTypeService;
...
import net.onlineshop.services.front.keyvalue.KeyvalueService;
...
import net.onlineshop.services.front.notifyTemplate.NotifyTemplateService;
...
import net.onlineshop.services.front.pay.PayService;
...
import net.onlineshop.services.manage.accountRank.AccountRankService;
import net.onlineshop.services.manage.accountRank.bean.AccountRank;
import net.onlineshop.services.manage.activity.ActivityService;
import net.onlineshop.services.manage.activity.bean.Activity;
import net.onlineshop.services.manage.hotquery.HotqueryService;
import net.onlineshop.services.manage.hotquery.bean.Hotquery;
import net.onlineshop.services.manage.oss.OssService;
...
//		}
	...
	public void loadPlugConfig() {
		...
		if(pay != null){
			...
			SystemManager.payCode = pay.getCode();
			 ...
			 */
			Pay yeePay = new Pay();
			yeePay.setCode(Pay.pay_code_yeepay);
			yeePay = payService.selectOne(yeePay);
			if(yeePay != null){
				SystemManager.yeepayConfig = yeePay.getSeller();
				SystemManager.payCode = yeePay.getCode();
				
				YeepayConfig.partner = yeePay.getPartner();
				YeepayConfig.key = yeePay.getKey1();
			}
		}
		...
		logger.debug("SystemManager.commentTypeCode="+SystemManager.commentTypeCode);
	}
...
//				logger.debug("SystemManager.attrsMap="+SystemManager.attrsMap);
//				logger.debug("SystemManager.attrsMap(63) = "+SystemManager.attrsMap.get("63"));
			}
		}
	}
		...
		try {
			...
			if(catalog.getPid().equals("0")){
				if(catalog.getChildren()==null){
					logger.debug(">>主类别catalog.getChildren()=0");
				}else{
					logger.debug(">>主类别catalog.getChildren()="+catalog.getChildren().size());
				}
				//主类别
...
//		loadCatalogs2();
		...
		logger.debug("SystemManager.catalogs="+SystemManager.catalogs.size());
		logger.debug("SystemManager.catalogsArticle="+SystemManager.catalogsArticle.size());
	...
	public void putToMap(List<Catalog> catalogs,boolean loadProduct) throws Exception{
		...
		for(int i=0;i<catalogs.size();i++){
			...
			if(SystemManager.catalogsCodeMap.get(item.getCode())!=null){
				logger.debug("item.code = " + item.getCode());
...
//			logger.debug("superMenuProducts = 0" + ",catalogCode = " + item.getCode());
//		}else{
//			logger.debug("superMenuProducts = " + superMenuProducts.size()  + ",catalogCode = " + item.getCode());
//		}
		if(superMenuProducts!=null && superMenuProducts.size()>0){
	...
	private void loadHotProductByCatalog(Catalog item){
		...
		if(hotProducts==null || hotProducts.size()==0){
			logger.debug("loadHotProductByCatalog.hotProducts = 0" + ",catalogCode = " + item.getCode());
		}else{
			logger.debug("loadHotProductByCatalog.hotProducts = " + hotProducts.size()  + ",catalogCode = " + item.getCode());
		}
...
//	}
	...
	public void readJsonArea(){
		...
		logger.debug("readJsonArea time = " + (System.currentTimeMillis() - start));
	}
...
//			
//			if(superMenuProducts==null || superMenuProducts.size()==0){
//				logger.debug("superMenuProducts = 0");
//			}else{
//				logger.debug("superMenuProducts = " + superMenuProducts.size());
//			}
//			if(superMenuProducts!=null && superMenuProducts.size()>0){
//				item.setSuperMenuProducts(superMenuProducts);
//			}
...
//	}
	...
	public void loadActivityProductList(){
		...
		if(list==null){
			logger.debug("loadActivityProductList.list=0");
		}else{
			logger.debug("loadActivityProductList.list="+list.size());
			for(int i=0;i<list.size();i++){
				Product p = list.get(i);
				Activity activity = SystemManager.activityMap.get(p.getActivityID());
				if(activity==null){
					logger.debug(" p = " + p.getId());
					continue;
				}
	...
	public void loadActivityScoreProductList(){
		logger.debug("loadActivityScoreProductList...");
		if(SystemManager.activityMap.size()==0){
			return;
		}
		...
		logger.debug("loadActivityScoreProductList...productIds="+productIds.toString());
...
//					p.setDiscountFormat(activity.getDiscountFormat());
	...
	public void loadActivityTuanProductList(){
		logger.debug("loadActivityTuanProductList...");
		if(SystemManager.activityMap.size()==0){
			return;
		}
