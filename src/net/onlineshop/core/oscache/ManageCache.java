...
import net.onlineshop.core.TaskManager;
...
import net.onlineshop.services.manage.task.TaskService;
...
public class ManageCache {
	...
	public void loadOrdersReport(){
		...
		logger.debug("SystemManager.ordersReport = " + SystemManager.ordersReport.toString());
	}
...
//	}
	...
	public void loadOSS() {
		...
		if(oss!=null){
			if(oss.getCode().equals(Oss.code_aliyun)){
				...
				SystemManager.isOSS = true;
			}
		}else{
			SystemManager.aliyunOSS = null;
			SystemManager.isOSS = false;
		}
	}
	...
	public void loadAllCache() throws Exception {
		logger.debug("ManageCache.loadAllCache...");
		loadOrdersReport();
//		readJsonArea();
		loadTask();
		loadOSS();
