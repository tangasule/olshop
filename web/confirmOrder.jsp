<%@page import="net.onlineshop.core.util.TokenUtil"%>
...
<%-- 												id:<s:property value="id" escape="false"/><br> --%>
			...
			<!-- 							<th >优惠方式(元)</th> -->
					...
					<%-- <%request.setAttribute("payCode", SystemManager.payCode);%>
					<c:if test="${payCode == 'yeepay' }">
						<li class="list-group-item">
							<span class="glyphicon"></span>&nbsp;请选择支付银行
						</li>
						<li class="list-group-item">
							<div class="row">
								<div class="col-xs-12">
									<%request.setAttribute("bankCodes", SystemManager.bankCodes); %>
									<table width="910px" height="325px" border="0">
										<c:forEach items="${bankCodes}" var="bankCode" varStatus="vs">
											<c:if test="${vs.index % 4==0}">
												<tr style="cursor: pointer;">
											</c:if>
											<td >
												<input type="radio" name="pd_FrpId" value="${bankCode}"  ${vs.count==1 ? 'checked' : '' } />
									    	    <img src="<%=request.getContextPath()%>/resource/images/bank/${bankCode}.gif" width="168" height="44"/>
											</td>
