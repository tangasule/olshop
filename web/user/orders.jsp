...
<!-- 							<table class="table table-bordered"> -->
	...
	<%-- 												<input type="checkbox" name="ids" value="<s:property value="id"/>" /> --%>
									...
									<s:iterator value="orders" status="i" var="ord">
										<tr>
											<td>&nbsp;
											...
											<s:if test="#i.index==0">
												<td style="text-align: center;border-left:1px solid #ddd;vertical-align: middle;" rowspan="<s:property value="quantity" />">
													...
													<s:if test="paystatus.equals(\"y\")">
														...
														<s:elseif test="status.equals(\"send\")">
															<c:choose>
																<c:when test="${ord.payType==1}">
																	<a target="_blank" href="http://www.alipay.com" class="btn btn-primary btn-sm">确认收货</a>
																</c:when>
																<c:otherwise>
																	<a target="_blank" href="<%=request.getContextPath()%>/orders!updateOrderStatus.action?e.id=<s:property value="id" />" class="btn btn-primary btn-sm">确认收货</a>
																</c:otherwise>
															</c:choose>
													...
													<s:if test="status.equals(\"send\") or status.equals(\"sign\")">
														<a target="_blank" href="http://www.kuaidi100.com/chaxun?com=<s:property value="expressCompanyName" />&nu=<s:property value="expressNo" />">快递物流</a>
													</s:if>
